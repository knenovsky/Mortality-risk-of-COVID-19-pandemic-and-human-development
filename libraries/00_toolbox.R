...
get_scaled_wdi_data <- function (qual_comp_res,
                                 wdi_data,
  ...
  wdi_data_cons_scaled <- scale(as.matrix(wdi_data_cons[, -1:-2]))
  return(wdi_data_cons_scaled)
}

save_csv <- function(csv, file_path) {
  # Check if the file already exists
  if (!file.exists(file_path)) {
    # If it doesn't exist, save the loadings to the specified path
    fwrite(file, file_path)
    cat("Loadings saved to", file_path, "\n")
  } else {
...
createXgboostData<-function(numberofruns=run,Xgboostinput_filtered="",features="",Label=""){
  wdi_imputed_shaptable<-data.frame()
  wdi_imputed_bootstrap<-data.frame()
  library(doMC)
  registerDoMC(30)

  for(i in numberofruns){
  ...
  folds<-createFolds(train_data$group, k = 10, list = TRUE)
  ...
  xgb_caret_wdi_raw <- caret::train(
    ...
    method = "xgbTree",trControl = custom_control,
    verbosity = 0,tuneLength=100
  )

  finalmodel_wdi_raw <- caret::train(
    x = train_data %>% dplyr::select(any_of(features)),
    y = train_data %>% pull(Label),
    method = "xgbTree",
  ...
  predictedValue<-postResample(
    predict(finalmodel_wdi_raw, 
            test_data %>% dplyr::any_of(features)),  
    test_data %>% pull(Label)
  )
  
  
  shaptable <- xgb.plot.shap.summary(data = as.matrix(train_data %>% select(features)), model = finalmodel_wdi_raw$finalModel, top_n =100)
  ...
  imp<-varImp(finalmodel_wdi_raw)$importance %>%
    ...
    filter(importance > 0)
  shaptable_data<-left_join(shaptable_data,imp,by="feature")
  shaptable_data$run<- i
  wdi_imputed_shaptable<-rbind(wdi_imputed_shaptable,shaptable_data)
  
  wdiImpPerf<-predictedValue  %>%  t() %>% data.frame()
  wdiImpPerf$shap_mean<-shaptable_data$Shap_value_mean %>%  sum
  wdiImpPerf$SeedNum<-seedNUM
  wdiImpPerf$`TestTraing`<-"20%"
  wdiImpPerf$run<- i
  wdiImpPerf<-cbind(wdiImpPerf,xgb_caret_wdi_raw$bestTune)
  wdi_imputed_bootstrap<-bind_rows(wdi_imputed_bootstrap,wdiImpPerf)
  }
  data_xg_output<-list(Xgboost_summary = wdi_imputed_bootstrap, Feature_summary = wdi_imputed_shaptable)
  return(data_xg_output)
}
shapplotresults2<-function(data, shap_contrib = NULL, features = NULL, top_n = 10, 
                           model = NULL, trees = NULL, target_class = NULL, approxcontrib = FALSE, 
                           subsample = NULL,inputcolor="orange") 
{
  ...
  supermax<-max(value2$max_positive_value)
  sumofsum<-value2$absSHAP %>%  sum()
  p <- ggplot2::ggplot(p_data, ggplot2::aes(x = feature, y = p_data$shap_value),alpha=0.8) + ggplot2::geom_jitter(alpha = 0.8, 
                                                                                                                  width = 0.1,color=inputcolor)+ 
    # ggplot2::geom_violin(alpha = 0.8, color=inputcolor,size=0.8)+
    ggplot2::geom_abline(slope = 0, 
                         intercept = 0, colour = "grey35") + 
    ...
    ggplot2::coord_flip()  + xlab("Features")+ylab("Shapley value")+theme(legend.position = "right")+labs(color = "Feature value")+ theme(
      legend.position = c(1, 0.5),
      legend.justification = c("right", "top"),
      legend.text = element_text(size=10),
      legend.box.just = "right",
...
shapplotresults_violin<-function(data, shap_contrib = NULL, features = NULL, top_n = 10, 
                           model = NULL, trees = NULL, target_class = NULL, approxcontrib = FALSE, 
                           subsample = NULL,inputcolor="orange") 
{
  require(xgboost)
...
shapplotresults1<-function(data, shap_contrib = NULL, features = NULL, top_n = 12, 
                           model = NULL, trees = NULL, target_class = NULL, approxcontrib = FALSE, 
                           subsample = NULL,onlyLegend=F) 
{
  require(xgboost)
  ...
  data_list <- xgboost:::xgb.shap.data(data = data, shap_contrib = shap_contrib, 
                                       ...
                                       subsample = subsample, max_observations = 10000)
  p_data <- xgboost:::prepare.ggplot.shap.data(data_list, normalize = TRUE)
  p_data$feature_value<-p_data$feature_value %>% scale(scale = 3,center = T)
  p_data[, `:=`("feature", factor(feature, rev(levels(feature))))]
  value<-p_data%>%  group_by(feature) %>% summarise(absSHAP=round(digits = 2,sum(abs(shap_value)))) %>%  arrange(desc(absSHAP))
  max_values <- p_data %>%
    group_by(feature) %>%
  ...
  supermax<-max(value2$max_positive_value)
  sumofsum<-value2$absSHAP %>%  sum()
  value2$absSHAP<-paste0(round((value2$absSHAP/sumofsum)*100,digits = 1)," %")
  
  p <- ggplot(p_data, ggplot2::aes(x = feature, y = p_data$shap_value,color = p_data$feature_value)) + ylim(-10,10)+
    geom_point(shape=3,size=4)+
    geom_boxplot(alpha = 0.5,color="grey25",size=0.5)+
    ...
    ggplot2::coord_flip()  + xlab("Features")+ylab("Shapley values")+theme(legend.position = "right")+labs(color = "Feature value")+ theme(
    ...
    #colorspace::choose_palette(low = "blue4",mid = "white",high = "firebrick4")+
    scale_color_continuous_diverging("Blue-Red 2")#+
    #scale_color_viridis(option="H")
  # ggplot2::geom_text(data = value2, aes(x = feature, y = supermax, label = absSHAP),
  #                    hjust = 1.1, vjust = -0.2, size = 3, fontface = "bold",color="black")
  p
}

createXgboostData_tuned<-function(numberofruns=run,Xgboostinput_filtered="",features="",Label=""){
  wdi_imputed_shaptable<-data.frame()
  wdi_imputed_bootstrap<-data.frame()
  # library(doParallel)
  ...
  for(i in numberofruns){
    ...
    param_grid <- expand.grid(
      ...
      max_depth = c(2, 3),     # Maximum tree depth
      eta = c(0.05),    # Learning rate
      gamma = c(1, 5),       # L2 regularization
    ...
    #   max_depth = c(2, 3),          # Shallow trees to avoid overfitting
    #   eta = c(0.01, 0.05),          # Slow learning rate for small data
    #   gamma = c(1, 5),              # Regularization to control complexity
    #   colsample_bytree = c(1),  # More aggressive feature subsampling
    #   subsample = c(0.8),      # More aggressive row subsampling
    #   min_child_weight = c(10, 15) # Require more data points per split to avoid overfitting
    # )
    # param_grid <- expand.grid(
    #   nrounds = c(50, 100),  # Number of boosting rounds
    #   max_depth = c(2, 3, 4),     # Maximum tree depth
    #   eta = c(0.1, 0.3),    # Learning rate
    #   gamma = c( 1, 10),       # L2 regularization
    #   colsample_bytree = c( 0.9,1),
    #   subsample = c( 0.9),  # Subsample ratio
    #   min_child_weight = c( 5, 10)  # Minimum child weight
    # )
    # Test different parameters
    ...
    xgb_caret_wdi_raw <- caret::train(
      x = train_data %>% dplyr::select(any_of(features)),
      y = train_data %>% pull(Label),
      method = "xgbTree",trControl = custom_control,tuneGrid=param_grid,
      verbosity = 0,tuneLength=100
    )
    ...
    overfitting_values<-(1- ( predictedValue[2]/finalmodel_wdi_raw$results$Rsquared))*100
    
    shaptable <- xgb.plot.shap.summary(data = as.matrix(train_data %>% select(any_of(features))), model = finalmodel_wdi_raw$finalModel, top_n =100)
    shaptable$data %>% group_by(feature) %>% summarise(numberobservations=length(shap_value),Shap_value_mean=mean(abs(shap_value))) %>%  arrange(desc(Shap_value_mean))->shaptable_data
    ...
    wdiImpPerf$`TestTraing`<-"20%"
    wdiImpPerf$run<- i
    wdiImpPerf$overfitt<-overfitting_values
    wdiImpPerf$trainR2<-finalmodel_wdi_raw$results$Rsquared
    wdiImpPerf<-cbind(wdiImpPerf,xgb_caret_wdi_raw$bestTune)
    wdi_imputed_bootstrap<-bind_rows(wdi_imputed_bootstrap,wdiImpPerf)
  }
  data_xg_output<-list(Xgboost_summary = wdi_imputed_bootstrap, Feature_summary = wdi_imputed_shaptable)
...
createXgboostData_test<-function(numberofruns=run,Xgboostinput_filtered="",features="",Label=""){
  wdi_imputed_shaptable<-data.frame()
  wdi_imputed_bootstrap<-data.frame()
...
createRandomforest_tuned<-function(numberofruns=run,Xgboostinput_filtered="",features="",Label=""){
  wdi_imputed_shaptable<-data.frame()
  wdi_imputed_bootstrap<-data.frame()
  # library(doParallel)
  # doParallel::registerDoParallel(30)
  ...
  for(i in numberofruns){
    ...
    #   gamma = c(0),       # L2 regularization
    #   colsample_bytree = c( 0.9),
    #   subsample = c(0.7),  # Subsample ratio
    ...
    xgb_caret_wdi_raw <- caret::train(
      x = train_data %>% dplyr::select(any_of(features)),
      y = train_data %>% pull(Label),
      method = "rf",trControl = custom_control,
      verbosity = 0,tuneLength=100
    )
  ...
  data_xg_output<-list(Xgboost_summary = wdi_imputed_bootstrap)
  return(data_xg_output)
}


createRandomForestData_tuned <- function(numberofruns = run, RFinput_filtered = "", features = "", Label = "") {
  library(iml)
  ...
  for (i in numberofruns) {
    ...
    # Split test training
    countries <- RFinput_filtered$Country.Code %>% unique()
    test_proportion <- 0.8
    train_countries <- sample(countries, size = length(countries) * test_proportion)
    train_data <- RFinput_filtered %>% filter(Country.Code %in% train_countries)
    test_data <- RFinput_filtered %>% filter(!(Country.Code %in% train_countries))
    train_data$group <- as.numeric(factor(train_data$Country.Code))
    ...
    # param_grid <- expand.grid(
    #   mtry = c(3, 5, 7),  # Number of variables randomly sampled as candidates at each split
    #   ntree = c(100, 200, 300),  # Number of trees in the forest
    #   maxnodes = c(10, 15, 20)   # Maximum number of terminal nodes in the tree
    # )
    # 
    # Train the Random Forest model using caret
    rf_caret_wdi_raw <- caret::train(
      x = train_data %>% dplyr::select(any_of(features)),
      y = train_data %>% pull(Label),
      method = "rf",
    ...
    finalmodel_wdi_raw <- caret::train(
      ...
      verbosity = 0,
      # trControl = custom_control,
      tuneGrid = rf_caret_wdi_raw$bestTune
    )
    ...
    # SHAP values calculation using iml package
    model <- Predictor$new(finalmodel_wdi_raw, data = train_data %>% dplyr::select(any_of(features)), y = train_data %>% pull(Label))
    shapley <- Shapley$new(model, x.interest = train_data %>% dplyr::select(any_of(features)))
    shap_values <- shapley$results
    
    # Summarize SHAP values
    shaptable_data <- shap_values %>%
      group_by(feature) %>%
      summarise(numberobservations = n(),
                Shap_value_mean = mean(abs(phi))) %>%
      arrange(desc(Shap_value_mean))
    ...
    # Merge SHAP summary and variable importance
    shaptable_data <- left_join(shaptable_data, imp, by = "feature")
    shaptable_data$run <- i
    wdi_imputed_shaptable <- rbind(wdi_imputed_shaptable, shaptable_data)
    ...
    wdiImpPerf$overfitt <- overfitting_values
    wdiImpPerf$trainR2 <- finalmodel_wdi_raw$results$Rsquared
    wdiImpPerf <- cbind(wdiImpPerf, rf_caret_wdi_raw$bestTune)
    wdi_imputed_bootstrap <- bind_rows(wdi_imputed_bootstrap, wdiImpPerf)
  }
  
  data_rf_output <- list(RF_summary = wdi_imputed_bootstrap, Feature_summary = wdi_imputed_shaptable)
  return(data_rf_output)
}
...
createXgboostData_tuned_testdata<-function(numberofruns=run,Xgboostinput_filtered="",features="",Label=""){
  wdi_imputed_shaptable<-data.frame()
  wdi_imputed_bootstrap<-data.frame()
  test_data_list<-data.frame()
  # library(doParallel)
  # doParallel::registerDoParallel(30)
  ...
  for(i in numberofruns){
    ...
    test_data$run<- i
    test_data$shap_mean<-shaptable_data$Shap_value_mean %>%  sum
    test_data$Rsquared<-wdiImpPerf$Rsquared[1]
    test_data$RMSE<-wdiImpPerf$RMSE[1]
    test_data_list<-bind_rows(test_data_list,test_data)
  }
  data_xg_output<-list(Xgboost_summary = wdi_imputed_bootstrap, Feature_summary = wdi_imputed_shaptable,test_subset=test_data_list)
  return(data_xg_output)
}
