...
LIB_DIR<-"../libraries"
WDI_DIR <- "../data/input_data/231103_WDI"
DIMRED_DIR<-"../data/01_data"
EXMORT_DIR <- "../data/input_data/230225_WHO_Excess_Mort/2022-03-25_covid-19_gem"
OUTPUT_DIR<- "../output"
...
#dimred_path<- "../../../closed_COCAP/Risk_and_resilience_original/data/01_data/01_wdi_data_cons_df_250.csv"
...
Peru out and others
```{r}
thiscountries<-dimred_250 %>%  filter(year%in%2020,!is.na(iso1)) %>% select(`Country Code`) %>%  unique 
...
### Filter peru
```{r}
countries_p_value_mort<-excessCounrty_annual %>%  filter(iso3%in% thiscountries$`Country Code`)
countries_p_value_mort[countries_p_value_mort$p_value_mort>median(countries_p_value_mort$p_value_mort)+IQR(countries_p_value_mort$p_value_mort)*4,]
IQR(countries_p_value_mort$p_value_mort)
median(countries_p_value_mort$p_value_mort)
```
