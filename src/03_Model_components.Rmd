...
OWID_DIR<-file.path(INPUT_DIR,"231103_OWID_COVID") ## https://ourworldindata.org/explorers/covid
OUTPUT_DIR<- "../output"
...
covar_path<-file.path(INPUT_DIR,"X_Covar_05_data.csv") # Dataset available on OSF (see Github)
...
covar<-fread(covar_path) %>% dplyr::select(any_of(feature_vector)) #avai
...
corrframe<-dimred_250 %>%  filter(year %in% 2021) %>% 
  dplyr::select(`Country Code`,dim_features,NY.GDP.PCAP.CD)  %>% 
...
vars <- names(corrframe)
n <- length(vars)

# Initialize an empty matrix
dcor_matrix <- matrix(NA, n, n)
colnames(dcor_matrix) <- rownames(dcor_matrix) <- vars

# Compute distance correlations pairwise
for (i in seq_len(n)) {
  for (j in seq_len(n)) {
    x <- corrframe[[i]]
    y <- corrframe[[j]]
    if (is.numeric(x) && is.numeric(y)) {
      dcor_matrix[i, j] <- dcor(x, y)
    }
  }
}
compareFrame<- dcor_matrix %>%  data.frame()
library(ggcorrplot)
a<-ggcorrplot(dcor_matrix,
           type = "upper",
           lab = FALSE,
           colors = viridisLite::viridis(100),
           tl.cex = 10) +
  scale_fill_viridis("Distance Correlation") +
  theme_classic() +
  theme(axis.text = element_text(size = 12),
        axis.text.x = element_text(angle = 90, hjust = 1)) +
  xlab("") + ylab("")+ 
scale_fill_viridis_c(option = "D", name = "Distance Correlation")
a
# a %>% ggsave(file="../figures/03_figures/03_Energy_Corr.png",device = "png",width = 9,height = 9)

```


```{r}
alfa<- round(abs(cor(corrframe)),digits = 3) %>%  data.frame
alfa
```
...
corrframe2<-dimred_250 %>%  filter(year %in% 2021) %>% 
  dplyr::select(`Country Code`,dim_features)  %>% 
  inner_join( .  ,owid_annual %>% filter(year%in% 2021),by=c("Country Code"="iso_code")) %>% 
  inner_join( . ,covar,by="Country Code")  %>% filter(!is.na(iso1),!is.na(airpolution_avg))%>% rename(
    "Urban area (%)" ="urban_percent", 
    "Med. Infr"= "Med_recources_m",
...
scale_fill_viridis_c(option = "D", name = "Distance Correlation")
b
# b%>% ggsave(file="../figures/03_figures/03_Energy_Corr_supplment.png",device = "png",width = 9,height = 9)
...
# Construct Versions of the integrated model 
##  PC 20 model
```{r}
...
Xgboostinput_complete_iso<-cbind(Xgboostinput_iso %>% dplyr::select(`Country Code`,iso1,iso2,iso3,iso4,iso5,iso6,iso7,iso8,iso9,iso10),Xboostinput_exmort_owid_covar_imputed)%>%
  dplyr::select(-matches("^PC_normal"))
...
```{r}
# pathWDI10_e_isomap<-file.path(INPUT_DIR,"03_data","03_Xgboost_E_iso10_models")
# 
# this10<-readRDS(pathWDI10_e_isomap)
topruns<-this10$Xgboost_summary %>% arrange(desc(Rsquared))
...
# Q3 Reviewer request why the broad range.
...
wdi_iso10Data<-createXgboostData_tuned_testdata(numberofruns = topruns$run[1:1000],Xgboostinput_filtered =Xgboostinput_iso,features = features,Label = Label)
  # gbossISO<-wdi_iso10Data$Xgboost_summary
# wdi_iso10Data$Xgboost_summary$Rsquared %>%  summary
# wdi_iso10Data$Xgboost_summary %>%  summary
#wdi_iso10Data %>% saveRDS("../output/03_2511_testdata")
wdi_iso10Data$Feature_summary%>% group_by(feature) %>% 
  reframe(value=sum(Shap_value_mean)) %>%  arrange(desc(value))
